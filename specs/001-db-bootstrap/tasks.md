...
- [X] T012 [P] [US1] Integration test validating successful bootstrap (entities + dependency graph + derived task runs/AI jobs) in `tests/integration/test_db_prepare_success.py`
- [X] T013 [P] [US1] Full project fixture containing dependencies, task-run hints, and AI job metadata in `tests/fixtures/projects/full_project/`
- [X] T014 [P] [US1] Golden datastore snapshot/assertions for success case in `tests/helpers/data_store_assertions.py`

### Implementation for User Story 1

- [X] T015 [P] [US1] Implement project parser reading project.md metadata in `src/services/parser/project_parser.py`
- [X] T016 [P] [US1] Implement feature parser that links features to projects in `src/services/parser/feature_parser.py`
- [X] T017 [P] [US1] Implement spec + task parser (status, acceptance criteria, dependency refs) in `src/services/parser/feature_parser.py`
- [X] T018 [P] [US1] Implement dedicated dependency parser that normalizes task dependency edges in `src/services/parser/dependency_parser.py`
- [X] T019 [US1] Build bootstrap orchestrator chaining discovery → parsing → validation → persistence in `src/services/bootstrap_orchestrator.py`
- [X] T020 [US1] Wire dry-run summary output (entity counts + pending writes + skip reasons) into `src/cli/commands/db_prepare.py`
- [X] T021 [US1] Implement upsert service for projects/features/specs/tasks in `src/services/upsert_service.py`
- [X] T022 [US1] Persist task dependencies via gateway + relationship mapper in `src/services/data_store_gateway.py`
- [X] T023 [US1] Implement task-run creation service (auto create unless skipped) in `src/services/task_run_service.py`
- [X] T024 [US1] Implement AI job creation service (auto create unless skipped) in `src/services/ai_job_service.py`
- [X] T025 [US1] Emit operation metrics + success report (per-entity totals, duration) in `src/lib/metrics.py`
...
- [x] T026 [P] [US2] Integration test covering circular dependency + duplicate IDs in `tests/integration/test_db_prepare_validation_errors.py`
- [x] T027 [P] [US2] Invalid documentation fixtures (missing metadata, duplicates, circular tasks) in `tests/fixtures/projects/invalid_project/`
- [x] T028 [P] [US2] Malformed markdown + missing frontmatter fixtures in `tests/fixtures/projects/malformed_docs/`
- [x] T029 [P] [US2] Integration test for malformed documentation detection in `tests/integration/test_db_prepare_malformed_docs.py`
- [x] T030 [P] [US2] Unit test simulating network/data-store failures with retry expectations in `tests/unit/test_db_prepare_network_failures.py`
- [x] T031 [P] [US2] Integration test detecting schema drift and surfacing remediation guidance in `tests/integration/test_db_prepare_schema_drift.py`

### Implementation for User Story 2

- [x] T032 [P] [US2] Implement validation rules for duplicates, circular dependencies, missing metadata, and malformed markdown in `src/services/validation/rules.py`
- [x] T033 [US2] Integrate validation pipeline into command preflight with severity gating + halt on high severity in `src/services/validation_pipeline.py`
- [x] T034 [US2] Build error reporter that surfaces file/line numbers, remediation suggestions, and grouped outputs in `src/lib/error_reporter.py`
- [x] T035 [US2] Format CLI output + exit codes for validation failures, including rollback summary blocks, in `src/cli/commands/db_prepare.py`
- [ ] T036 [US2] Add rollback summary emitter that lists reverted operations in `src/services/rollback_manager.py`
- [x] T037 [US2] Implement network-failure retry/backoff + observability hooks in `src/services/data_store_gateway.py`
- [x] T038 [US2] Detect schema drift (unexpected columns/entities) and surface remediation guidance in `src/services/data_store_gateway.py`
...
- [x] T039 [P] [US3] Integration test executing command thrice and asserting stable counts in `tests/integration/test_db_prepare_idempotent.py`
- [ ] T040 [P] [US3] Shared datastore assertion helpers for before/after snapshots in `tests/helpers/data_store_assertions.py`
- [x] T041 [P] [US3] Integration test for `--force` overwrite workflow in `tests/integration/test_db_prepare_force_mode.py`

### Implementation for User Story 3

- [x] T042 [P] [US3] Implement entity matcher resolving existing records via codes/checksums in `src/services/matchers/entity_matcher.py`
- [x] T043 [US3] Extend upsert service with idempotent diff + merge logic in `src/services/upsert_service.py`
- [x] T044 [US3] Enforce concurrency-safe queues + retries in `src/lib/locking.py`
- [x] T045 [US3] Emit idempotency metrics (unchanged counts, retries) in `src/lib/metrics.py`
- [x] T046 [US3] Implement force-mode reconciliation (audit log + warning output) in `src/services/upsert_service.py`
...
- [x] T047 [P] [US4] Unit test covering CLI option combinations + validation in `tests/unit/test_db_prepare_flags.py`
- [x] T048 [P] [US4] Scoped project fixtures for selective bootstrap scenarios in `tests/fixtures/projects/scoped_project/`

### Implementation for User Story 4

- [x] T049 [P] [US4] Implement option parsing + validation for skip/project flags in `src/cli/options/db_prepare_options.py`
- [x] T050 [US4] Update bootstrap orchestrator to branch entity processing per flag set in `src/services/bootstrap_orchestrator.py`
- [x] T051 [US4] Add selective persistence filters (per-entity toggles) in `src/services/data_store_gateway.py`
- [x] T052 [US4] Document usage examples + flag matrix in `docs/cli/db_prepare.md`
